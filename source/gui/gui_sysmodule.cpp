...
void GuiSysmodule::draw() {
    ...
    if (anyModulesPresent)
        Gui::drawTextAligned(font20, Gui::g_framebuffer_width / 2, 150, currTheme.textColor, "选择要运行的后台服务(系统插件)。\n 由于内存限制，可能无法同时启动所有服务。", ALIGNED_CENTER);
    else
        Gui::drawTextAligned(font20, Gui::g_framebuffer_width / 2, 550, currTheme.textColor, "您目前未安装任何受支持的系统插件。\n 如需使用此功能，请安装受支持的系统插件(NSP格式)。", ALIGNED_CENTER);
